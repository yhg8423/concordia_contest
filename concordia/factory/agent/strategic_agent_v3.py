...
class EvaluationOfTheOptions(action_spec_ignored.ActionSpecIgnored):
  ...
  def _make_pre_act_value(self) -> str:
    ...
    reflection_on_the_options_result = reflection_on_the_options.open_question(
      question=(
        f"Considering the above memories, observations, and the characteristics of the current scenario, please reflectively evaluate {agent_name}'s options based on previous actions and decisions (not excersie, only act) from a your behavior (loss aversion or opportunist) perspective and game theory perspective."
      ),
      max_tokens=1000,
      terminators=(),
    )
