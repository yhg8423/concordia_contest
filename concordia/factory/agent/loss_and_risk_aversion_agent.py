...
class RiskEvaluation(agent_components.action_spec_ignored.ActionSpecIgnored):
  ...
  def _make_pre_act_value(self) -> str:
    ...
    risk_evaluation_result = f"{agent_name} thinks that ".format(agent_name=agent_name) + risk_evaluation_result
...
def build_agent(
  ...
  loss_and_risk_minimize_option_perception = (
      agent_components.question_of_recent_memories.QuestionOfRecentMemories(
          model=model,
          components=loss_minimize_option_perception,
          clock_now=clock.now,
          pre_act_key=loss_and_risk_minimize_option_perception_label,
          question=(
              f"Considering the statements above, which of {agent_name}'s options "
              "has the highest likelihood of avoiding potential losses and risks? If multiple "
              "options offer the same level of loss avoidance, select the option "
              f"that {agent_name} thinks will minimize losses and risks most quickly and "
              "most certainly."
          ),
