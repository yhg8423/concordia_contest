...
class ProfitEvaluation(agent_components.action_spec_ignored.ActionSpecIgnored):
  ...
  def __init__(
      ...
      pre_act_key: str = 'Profit Evaluation',
  ...
  def _make_pre_act_value(self) -> str:
    ...
    profit_evaluation_result = prompt.open_question(
      question=(
        ...
        f"each option. Please answer in the format `{agent_name} thinks that the profit of option X is Y, because ..., and the profit of option Z is W, because ...` "
        f"For example, `{agent_name} thinks that the profit of option X is 4, because ..., and the profit of option Z is 7, because ...`"
      ),
...
def build_agent(
  ...
  profit_pursuit_option_perception_label = (
    ...
    f'would best gain potential profit for {agent_name} right now?\nAnswer')
  ...
  profit_pursuit_option_perception = (
      agent_components.question_of_recent_memories.QuestionOfRecentMemories(
          ...
          question=(
              ...
              "has the highest likelihood of gaining potential profit? If multiple "
              "options offer the same level of profit gain, select the option "
              f"that {agent_name} thinks will gain profit most quickly and "
              "most certainly."
          ),
