...
# class LossEvaluation(agent_components.question_of_recent_memories.QuestionOfRecentMemories):
#   """This component evaluates the loss of each option for the agent."""

#   def __init__(self, **kwargs):
#     super().__init__(
#         question=(
#             "For each option {agent_name} is considering, evaluate the loss "
#             "that {agent_name} would incur if they chose that option on a "
#             "scale of 0 to 10. Provide a score and a brief explanation for "
#             "each option. Please answer in the format `{agent_name} thinks that the loss of option X is Y, because ..., and the loss of option Z is W, because ... Therefore, to achieve loss aversion, {agent_name} should choose option ...` "
#             "For example, `{agent_name} thinks that the loss of option X is 4, because ..., and the loss of option Z is 7, because ... Therefore, to achieve loss aversion, {agent_name} should choose option ...`"
#         ),
#         answer_prefix="{agent_name} thinks that ",
#         add_to_memory=False,
#         memory_tag='[loss evaluation]',
#         **kwargs,
#     )
...
class LossEvaluation(agent_components.action_spec_ignored.ActionSpecIgnored):
  ...
  def _make_pre_act_value(self) -> str:
    ...
    loss_evaluation_result = prompt.open_question(
      question=(
        f"For each option {agent_name} is considering, evaluate the economic and financial loss "
        "that {agent_name} would incur if they chose that option on a "
        "scale of 0 to 10. Provide a score and a brief explanation for "
        "each option, then explain which option has the lowest loss."
      ),
      answer_prefix=f"Loss evaluation result: ",
      max_tokens=1000,
      terminators=(),
    )
...
def build_agent(
  ...
  economic_loss_aversion_label = f'\n{agent_name}\'s Economic Loss Aversion '
  economic_loss_aversion = agent_components.constant.Constant(
    state=(f'{agent_name} exhibits strong loss aversion tendencies based on '
           f'prospect theory, particularly in economic and financial matters. {agent_name} is much more sensitive to potential '
           ...
           f'decision-making, especially regarding resources and assets.\n'
           f'{agent_name} tends to overvalue what {agent_name} already '
           f'possesses and is reluctant to part with it (the endowment '
           f'effect). {agent_name} often prefers avoiding financial losses to '
           f'acquiring equivalent gains. For instance, {agent_name} would '
           f'rather not lose $100 than gain $100.\n'
           f'When faced with an economic choice, {agent_name} always frames it in '
           f'terms of potential monetary losses rather than potential gains. {agent_name} '
           f'is risk-averse for financial gains but risk-seeking for losses. This '
           f'means {agent_name} prefers a sure monetary gain over a larger but '
           ...
           f'economic gains and losses rather than final outcomes. {agent_name} '
           f'often uses mental accounting, treating money differently '
           f'depending on its origin and intended use, rather than thinking '
           f'of it as fungible.\n'
           f'In business negotiations or financial conflicts, {agent_name} is more motivated '
           f'by the fear of losing assets than the possibility of gaining them. {agent_name} '
           f'may seem overly cautious or even paranoid about potential '
           f'economic losses. However, when {agent_name} feels they are already losing money '
           f'or resources, {agent_name} might become surprisingly risk-seeking in '
           f'an attempt to avoid or recover from the financial loss.\n'
           f'To rationalize {agent_name}\'s economic behavior, {agent_name} often '
           f'comes up with seemingly logical reasons for their choices, even '
           f'though these choices are primarily driven by financial loss aversion. '
           f'{agent_name} might claim to be "fiscally prudent" or "financially careful" rather '
           f'than admitting to fear of monetary loss.\n'
           f'{agent_name} believes this approach will help {agent_name} '
           f'minimize financial losses and maintain {agent_name}\'s economic position, '
           f'even if it means missing out on potential monetary gains.'),
    pre_act_key=economic_loss_aversion_label,
    logging_channel=measurements.get_channel('EconomicLossAversion').on_next)
  ...
  options_perception_components.update({
      ...
      economic_loss_aversion_label: economic_loss_aversion_label,
  ...
  loss_minimize_option_perception_label = (
      ...
      f'would best avoid potential economic and financial losses for {agent_name} right now?\nAnswer')
  ...
  loss_minimize_option_perception = (
      agent_components.question_of_recent_memories.QuestionOfRecentMemories(
          ...
          question=(
              ...
              "has the highest likelihood of avoiding potential economic and financial losses? If multiple "
              "options offer the same level of loss avoidance, select the option "
              f"that {agent_name} thinks will minimize economic and financial losses most quickly and "
              "most certainly."
          ),
  ...
  components_of_agent[economic_loss_aversion_label] = economic_loss_aversion
  component_order.insert(
    component_order.index(_get_class_name(observation_summary)) + 1,
    economic_loss_aversion_label)
