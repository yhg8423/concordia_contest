...
class EvaluationOfTheOptions(action_spec_ignored.ActionSpecIgnored):
  """This component evaluates the options available to the agent from a loss aversion perspective or opportunist perspective."""
  ...
  def __init__(
      ...
      selection_loss_aversion_or_opportunist_component_name: str,
    ...
    self._selection_loss_aversion_or_opportunist_component_name = selection_loss_aversion_or_opportunist_component_name
  ...
  def _make_pre_act_value(self) -> str:
    ...
    selection_loss_aversion_or_opportunist_component = self.get_entity().get_component(
      self._selection_loss_aversion_or_opportunist_component_name,
      type_=agent_components.constant.Constant
    )
    selection_loss_aversion_or_opportunist = selection_loss_aversion_or_opportunist_component.get_pre_act_value()
    ...
    what_is_the_behavior_of_the_agent = interactive_document.InteractiveDocument(self._model)
    what_is_the_behavior_of_the_agent.statement(f'{agent_name}\'s behavior: {selection_loss_aversion_or_opportunist}\n')
    what_is_the_behavior_of_the_agent = what_is_the_behavior_of_the_agent.open_question(
      question=(
        f"What is the behavior of {agent_name} in game theory perspective? Please answer with only words `opportunist` or `loss aversion`. "
        f"For instance, if {agent_name} is loss averse, answer `loss aversion\n`. If {agent_name} is opportunist, answer `opportunist\n`."
      ),
      max_tokens=200,
      terminators=('\n',),
    )

    behavior_of_the_agent = 'loss aversion'
    if 'opportunist' in what_is_the_behavior_of_the_agent.lower():
      behavior_of_the_agent = 'opportunist'
    elif 'loss aversion' in what_is_the_behavior_of_the_agent.lower():
      behavior_of_the_agent = 'loss aversion'
    else:
      behavior_of_the_agent = what_is_the_behavior_of_the_agent
    ...
    if behavior_of_the_agent == 'loss aversion':
      reflection_on_the_options_result = reflection_on_the_options.open_question(
        question=(
          f"Considering the above memories, observations, and the characteristics of the current scenario, please reflectively evaluate {agent_name}'s options based on previous actions and decisions (not excersie, only act) from loss aversion perspective and game theory perspective."
        ),
        max_tokens=1000,
        terminators=(),
      )
    elif behavior_of_the_agent == 'opportunist':
      reflection_on_the_options_result = reflection_on_the_options.open_question(
        question=(
          f"Considering the above memories, observations, and the characteristics of the current scenario, please reflectively evaluate {agent_name}'s options based on previous actions and decisions (not excersie, only act) from opportunist perspective and game theory perspective."
        ),
        max_tokens=1000,
        terminators=(),
      )
      ...
      reflection_on_the_options_result = reflection_on_the_options.open_question(
        question=(
          f"Considering the above memories, observations, and the characteristics of the current scenario, please reflectively evaluate {agent_name}'s options based on previous actions and decisions (not excersie, only act) from {behavior_of_the_agent} perspective and game theory perspective."
        ),
        max_tokens=1000,
        terminators=(),
      )
    ...
    if behavior_of_the_agent == 'loss aversion':
      evaluation_of_the_options_result = prompt.open_question(
        question=(
          ...
          f"scale of 0 to 10 in game theory perspective. "
          f"Provide a score and a brief explanation for "
          f"each option, then explain which option has the lowest loss."
        ),
      ...
      evaluation_of_the_options_result = prompt.open_question(
        ...
        answer_prefix=f"Evaluation result: ",
        max_tokens=1000,
        terminators=(),
      )
...
def build_agent(
  ...
  evaluation_of_the_options_label = (
      f'\nQuestion: For each option {agent_name} is considering, evaluate the option on a '
      f'scale of 0 to 10 in game theory perspective. Provide a score and a brief explanation for each option, then explain which option is the best.\nAnswer')
  evaluation_of_the_options = EvaluationOfTheOptions(
    model=model,
    observation_component_name=_get_class_name(observation),
    options_perception_component_name=_get_class_name(options_perception),
    selection_loss_aversion_or_opportunist_component_name=_get_class_name(selection_loss_aversion_or_opportunist),
    components={
      _get_class_name(observation): observation_label,
      _get_class_name(observation_summary): observation_summary_label,
      _get_class_name(relevant_memories): relevant_memories_label,
